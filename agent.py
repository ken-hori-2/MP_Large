...
class Agent():
    ...
    def back_position(self, test_bp_st, Attribute): # change
        ...
        # print("----- 👀🤖👀 -----")
        # print(Attribute["move cost"])
        # max_num = max(Attribute["move cost"])
        max_num = Attribute["move cost"].max(skipna=True)
        # print("max cost:", max_num)
        # print(Attribute["move cost"]/max_num)

        Attribute["move cost"] = Attribute["move cost"]/max_num # 正規化

        # print("-----")
        # print(Attribute["stress"])
        # max_s = max(Attribute["stress"])
        # Attribute["stress"] = np.array(Attribute["stress"])
        # max_s = np.array(Attribute["stress"]).nanmax()
        max_s = Attribute["stress"].max(skipna=True)
        # print("max stress:", max_s)
        # print(Attribute["stress"]/max_s)
